...
int handler::ha_write_row(const uchar *buf)
{
  ...
  */
  ...
  DBUG_ASSERT(write_partition || this == table->file);

  /*
    MERGE: remove this comment, resolve in favor of 10.4:
           keep !write_partition instead of this == table->file
  */
...
int handler::ha_update_row(const uchar *old_data, const uchar *new_data)
{
  ...
  */
  const bool update_partition= table->part_info &&
                               (ht->db_type != DB_TYPE_PARTITION_DB);
  DBUG_ASSERT(update_partition || this == table->file);

  /*
    MERGE: remove this comment, resolve in favor of 10.4:
           keep !update_partition instead of this == table->file
  */
